...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('⚡𝘉𝘰𝘵 𝘖𝘸𝘯𝘦𝘳', callback_data="owner_info"),
                    InlineKeyboardButton('⌬ 𝘚𝘶𝘱𝘱𝘰𝘳𝘵 𝘎𝘳𝘰𝘶𝘱', url=GRP_LNK)
                ],[
                    ...
                    InlineKeyboardButton('✇ 𝘑𝘰𝘪𝘯 𝘜𝘱𝘥𝘢𝘵𝘦𝘴 𝘊𝘩𝘢𝘯𝘯𝘦𝘭 ✇', url=CHNL_LNK)
                  ]]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "❆ Jᴏɪɴ Oᴜʀ Bᴀᴄᴋᴜᴘ Cʜᴀɴɴᴇʟ ❆", url=invite_link.invite_link
                )
            ]
        ]
    ...
    if not files_:
        ...
        try:
            if IS_VERIFY and not await check_verification(client, message.from_user.id):
                btn = [[
                    InlineKeyboardButton("🥂Vᴇʀɪғʏ🥂", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start=", file_id)),InlineKeyboardButton("⚡ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ⚡", url="https://t.me/aboutme_tonystark/3"))
                ]]
                await message.reply_text(
